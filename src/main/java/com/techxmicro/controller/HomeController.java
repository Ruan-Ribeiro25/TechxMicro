...
import com.techxmicro.entity.Chamado;
...
import com.techxmicro.repository.ChamadoRepository;
import com.techxmicro.repository.InteracaoChamadoRepository;
...
public class HomeController {
    ...
    /**
     * ROTA DE RECEPÇÃO (ROOT)
     * Agora retorna a página institucional da TechxMicro.
     */
    @GetMapping("/")
    public String root(Model model, Principal principal) {
        ...
        return "recepcao"; // Carrega templates/recepcao.html
    }
    ...
    public String helpdesk(@RequestParam(defaultValue = "0") int page, Model model, Principal principal) {
        ...
        int[] infraResolvidosTime = new int[4];
        int[] infraPendentesTime = new int[4];
        int[] infraCanceladosTime = new int[4];
        ...
        for (Chamado c : todosChamados) {
            if (c.getDataAbertura() != null) {
                ...
                if (c.getCategoria() == CategoriaChamado.TI) {
                    ...
                    if (isHoje) {
                        if (c.getStatus() == StatusChamado.RESOLVIDO) infraResolvidosTime[0]++;
                        else if (c.getStatus() == StatusChamado.PENDENTE) infraPendentesTime[0]++;
                        else if (c.getStatus() == StatusChamado.CANCELADO) infraCanceladosTime[0]++;
                    }
                    if (isSemanaAtual) {
                        if (c.getStatus() == StatusChamado.RESOLVIDO) infraResolvidosTime[1]++;
                        else if (c.getStatus() == StatusChamado.PENDENTE) infraPendentesTime[1]++;
                        else if (c.getStatus() == StatusChamado.CANCELADO) infraCanceladosTime[1]++;
                    }
                    if (isAnoAtual) {
                        if (c.getStatus() == StatusChamado.RESOLVIDO) infraResolvidosTime[2]++;
                        else if (c.getStatus() == StatusChamado.PENDENTE) infraPendentesTime[2]++;
                        else if (c.getStatus() == StatusChamado.CANCELADO) infraCanceladosTime[2]++;
                    }
                    if (diffAno >= 0 && diffAno < 10) {
                        if (c.getStatus() == StatusChamado.RESOLVIDO) infraResolvidosTime[3]++;
                        else if (c.getStatus() == StatusChamado.PENDENTE) infraPendentesTime[3]++;
                        else if (c.getStatus() == StatusChamado.CANCELADO) infraCanceladosTime[3]++;
                    }
                } 
        ...
        model.addAttribute("infraResolvidosTime", infraResolvidosTime);
        model.addAttribute("infraPendentesTime", infraPendentesTime);
        model.addAttribute("infraCanceladosTime", infraCanceladosTime);
    ...
    /**
     * GESTÃO DE CHAMADOS (NOVO, VISUALIZAR, ASSUMIR)
     */
    ...
    public String atualizarStatus(@RequestParam("id") Long id, 
        ...
        if (chamado != null) {
            ...
            if (!podeInteragir) {
                redirectAttributes.addFlashAttribute("erroMensagem", "Acesso Negado: Apenas o solicitante, o técnico responsável ou a administração podem alterar o status.");
    ...
    public String responderChamado(@RequestParam("chamadoId") Long chamadoId,
        ...
        try {
            ...
            if (!podeResponder) {
                redirectAttributes.addFlashAttribute("erroMensagem", "Modo Espectador: Apenas os envolvidos diretos podem enviar mensagens.");
            ...
            if (texto != null && !texto.trim().isEmpty()) {
                ...
                interacao.setDataEnvio(LocalDateTime.now(ZoneId.of("America/Sao_Paulo")));
    ...
    public String auditoriaHelpdesk(Model model, Principal principal) {
        ...
        if (!isAdmin) return "redirect:/helpdesk?erroAuditoria=true";
    ...
    public ResponseEntity<ByteArrayResource> downloadAuditoriaHelpdesk(@PathVariable String formato, Principal principal) {
        ...
        try {
            if ("JSON".equalsIgnoreCase(formato)) {
                ...
                StringBuilder sb = new StringBuilder();
                sb.append("=== AUDITORIA DE CHAMADOS TECHXMICRO ===\n");
    ...
    private List<Object[]> gerarLinhaDoTempoHelpdesk() {
        ...
        for (Chamado c : chamados) {
            logsHelpdesk.add(new Object[]{
                ...
                c.getSolicitante() != null ? c.getSolicitante().getNome() : "Desconhecido"
            });
            ...
            if (c.getInteracoes() != null) {
                for (InteracaoChamado i : c.getInteracoes()) {
                    logsHelpdesk.add(new Object[]{
                        ...
                        i.getAutor() != null ? i.getAutor().getNome() : "Desconhecido"
                    });
                }
            }
        }

        logsHelpdesk.sort((a, b) -> ((LocalDateTime) b[0]).compareTo((LocalDateTime) a[0]));
        return logsHelpdesk;
    }
}